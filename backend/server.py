...
async def startup():
    ...
    default_departments = ["MUHASEBE", "İHRACAT", "ÜRETİM&OPERASYON", "YÖNETİM"]
    for dept_name in default_departments:
        ...
        if not existing:
            await db.departments.insert_one({
                "id": str(uuid.uuid4()),
                "name": dept_name,
                "description": f"{dept_name} departmanı",
                "created_at": datetime.now(timezone.utc).isoformat()
            })
    ...
    admin_email = os.environ.get("ADMIN_EMAIL", "ekrem.karabiyik").lower()
